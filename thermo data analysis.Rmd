...
#| label: find-work
w1 <- trial1 |>
  mutate(w = w_dot * (time - lag(time)) * 60) |>
  select(time, w)
w2 <- trial2 |>
  mutate(w = w_dot * (time - lag(time)) * 60) |>
  select (time, w)
...
total_q <- all |>
  group_by(trial) |>
  summarise(
    q_f_total = sum(abs(q_f), na.rm = TRUE),
    q_w_total = sum(abs(q_w), na.rm = TRUE),
    qt = sum(abs(q_f), na.rm = TRUE) + sum(abs(q_w), na.rm = TRUE),
    wt = sum(abs(w), na.rm = TRUE)) 
total_q
```
