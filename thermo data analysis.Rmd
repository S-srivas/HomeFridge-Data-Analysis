...
q_water_1 <- trial1 |>
  mutate(
    dT_dt = (tw - lag(tw)) / ((time - lag(time)) * 60), # Degrees per second
    q_w = abs(0.25 * 4.184 * dT_dt)  
    )
q_water_2 <- trial2 |>
  mutate(
    dT_dt = (tw - lag(tw)) / ((time - lag(time)) * 60), # Degrees per second
...
q_frid1 <- trial1 |>
  mutate(
    dT_dt = (tfrid - lag(tfrid)) / ((time - lag(time)) * 60), # Degrees per second
    q_w = abs(0.25 * 1.005 * dT_dt)  
    )
q_frid2 <- trial2 |>
  mutate(
    dT_dt = (tfrid - lag(tfrid)) / ((time - lag(time)) * 60), # Degrees per second
...
combined_q_1 <- q_water_1 |>
  ...
  select(time, w_dot, w, q_f, q_w, q_f_dot, trial)
...
#all|>
#  group_by(trial)|>
#  summarize(q_total = sum(q_w_dot), q_lost_w = mean(q_w))
```
